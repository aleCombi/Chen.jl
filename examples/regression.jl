...
function call_option_regression_experiment(;
    ...
    signature_level = 5,
    payoff_func = (path, strike) -> max(path[end][2] - strike, 0),  # default: call payoff
    verbose = false,
...
call_payoff = (path, K) -> max(path[end][2] - K, 0.0)
...
common_params = (
    ...
    K = 1.0,
    n_steps = 10,
    payoff_func = asian_arith_call,  # Call option payoff
    verbose = false,
    μ = 0.04,
    σ = 0.5
)
...
# Ridge
result_ridge = call_option_regression_experiment(;
    common_params...,
    regression_method = :ridge
);

# Lasso
result_lasso = call_option_regression_experiment(;
    common_params...,
...
println("="^60)
println("Method      Train R²   Test R²    Test RMSE   Gap")
println("-"^60)
println("OLS         $(rpad(round(result_ols.test_r2, digits=4), 10)) $(rpad(round(result_ols.test_r2, digits=4), 10)) $(rpad(round(result_ols.test_rmse, digits=4), 11)) $(round(result_ols.train_r2 - result_ols.test_r2, digits=4))")
println("Ridge       $(rpad(round(result_ridge.train_r2, digits=4), 10)) $(rpad(round(result_ridge.test_r2, digits=4), 10)) $(rpad(round(result_ridge.test_rmse, digits=4), 11)) $(round(result_ridge.train_r2 - result_ridge.test_r2, digits=4))")
println("Lasso       $(rpad(round(result_lasso.train_r2, digits=4), 10)) $(rpad(round(result_lasso.test_r2, digits=4), 10)) $(rpad(round(result_lasso.test_rmse, digits=4), 11)) $(round(result_lasso.train_r2 - result_lasso.test_r2, digits=4))")
println("="^60)